...
from ReES.classification import BasicFlattening, GLiNERClassificationModel
...
model_classification = GLiNERClassificationModel(BasicFlattening(), threshold=gliner_threshold)
...
async def process_texts(
    ...
    for i, data in enumerate(texts, index_offset):
        knowledge_base = Neo4jKnowledgeBase(f"{tenant_identifier}{token_size}K{qa.value}Q{i}")
